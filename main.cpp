...
void move_snake(sf::Sprite& inky_head_sprite);
...
int main()
{
    ...
    if (!head_texture.loadFromFile("head.png"))  return -1;
    sf::Sprite inky_head_sprite(head_texture);
    head_texture.setSmooth(true);
    sf::FloatRect bounds = inky_head_sprite.getLocalBounds();
    inky_head_sprite.setOrigin(bounds.getCenter());
    ...
    inky_head_sprite.setScale(sf::Vector2f (head_scale_x, head_scale_y));
    ...
    while (window.isOpen())
    {
        ...
        switch (current_state) {
            case GameState::GAME_PLAY: {
                ...
                move_snake(inky_head_sprite);
                window.draw(sh.shape);
                inky_head_sprite.setPosition(sf::Vector2f(sh.shape.getPosition().x + 20, sh.shape.getPosition().y + 20));
                window.draw(inky_head_sprite);
